...
By default the Better Auth component uses Convex's [`customJwt`](https://docs.convex.dev/auth/advanced/custom-jwt) to avoid the OIDC request by providing the JWKS URL statically in the auth config. Static JWKS goes further, avoiding both calls by providing the JWKS itself as a data URI.
